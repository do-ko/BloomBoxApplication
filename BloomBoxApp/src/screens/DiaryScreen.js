            ...
            <View style={styles.imageNameContainer}>
                ...
                <View style={styles.imageContainer}>
                    ...
                    <Pressable style={styles.editButton}
                               onPress={() => navigation.navigate("EditDiary", {diary, plant})}>
                        <BigEditSvg/>
                    </Pressable>
                    ...
                    <View style={styles.nameDataContainer}>
                        <View style={styles.diaryDateContainer}>
                            <Text
                                style={styles.diaryDateText}>{new Date(Date.parse(diary.entryDate)).getDate() < 10 ? "0" + new Date(Date.parse(diary.entryDate)).getDate() : new Date(Date.parse(diary.entryDate)).getDate()}</Text>
                            <Text
                                style={styles.diaryDateMonthText}>{new Date(Date.parse(diary.entryDate)).toLocaleString('default', {month: 'long'})}</Text>
                            <Text
                                style={styles.diaryDateText}>{new Date(Date.parse(diary.entryDate)).getFullYear()}</Text>
                        </View>

                        <View style={styles.diaryDateContainer}>
                            {diary.title.length <= 14 ? <Text style={styles.diaryTitleText(24)}>{diary.title}</Text> :
